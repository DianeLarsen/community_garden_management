...
export async function POST(request) {
  ...
  try {
    ...
    const plotQuery = `
      SELECT garden_id, p.id
      FROM garden_plots p
      LEFT JOIN group_memberships gm ON gm.group_id = p.group_id
      WHERE p.id = $1 AND (p.user_id = $2 OR (gm.user_id = $2 AND p.group_id = $3))
    `;
    ...
    const garden_id = plotResult.rows[0].garden_id;
    ...
    // Insert event creator as admin in event_registrations
    const registrationQuery = `
      INSERT INTO event_registrations (user_id, event_id, group_id, role)
      VALUES ($1, $2, $3, 'admin')
    `;
    await client.query(registrationQuery, [user_id, eventId, group_id || null]);
